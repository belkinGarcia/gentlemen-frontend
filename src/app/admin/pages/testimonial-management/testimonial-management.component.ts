// src/app/admin/pages/testimonial-management/testimonial-management.component.ts
...
import { MatTooltipModule } from '@angular/material/tooltip'; // Add MatTooltipModule for UX
...
@Component({
  ...
  imports: [
    ...
    MatIconModule,
    MatChipsModule,
    MatTooltipModule // Added for better UX
  ],
...
export class TestimonialManagementComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    // Subscribes to ALL testimonials (pending, approved, rejected)
    this.testimonialSub = this.testimonialService.getAllTestimonials().subscribe(testimonials => {
      // Order: Pending first, then by date (newest first)
