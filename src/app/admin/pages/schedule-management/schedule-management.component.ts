// src/app/admin/pages/schedule-management/schedule-management.component.ts
...
import { BarberService, Barber, WorkSchedule } from '../../../services/barber.service'; // <-- Importa WorkSchedule
...
export class ScheduleManagementComponent implements OnInit, OnDestroy {
  ...
  timeSlots: string[] = [];
  workSchedule: DaySchedule[] = []; // El horario que se guarda/edita
  ...
  ngOnInit(): void {
    // 1. Inicializa la estructura de horarios con días vacíos.
    ...
    this.route.paramMap.subscribe(params => {
      ...
      if (this.barberId) {
        ...
        this.dataSub = this.reservationService.getReservations().subscribe(reservations => {
          ...
          this.loadScheduleData(this.selectedDate); // Recarga para ver citas de hoy
        });
      }
    });
  }
  ...
  // Lógica de carga de disponibilidad para el calendario (no cambia)
  public loadScheduleData(date: Date): void {
    this.selectedDate = date;

// --- ¡CORRECCIÓN! AÑADIDO EL TERCER ARGUMENTO ---
  ...
  // --- Lógica de gestión de horario (para la tabla de 24h) ---
  ...
  saveWorkSchedule(): void {
    ...
    // Llama al método del BarberService para guardar la agenda fija
  ...
  // Helpers para el template (no cambian)
